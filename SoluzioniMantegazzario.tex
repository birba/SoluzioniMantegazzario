...
$$
\left\langle\frac{a}{b}\, x|\frac{c}{d}\,  y\right\rangle=\frac{1}{b^2d^2}\left\langle ad\, x|cb\, y\right\rangle=\frac{ac}{bd}\left\langle x|y\right\rangle\qquad a,b,c,d \in \NN
$$
questo prova l' omogeneità sui razionali, per continuità ciò è vero su $\RR$. 
\ex{6.39} Siano $\Norm{\cdot}_1$ e $\Norm{\cdot}_2$ due norme. Prendo $B_1\subseteq V$ la sfera unitaria rispetto alla prima (che in dimensione finita è sempre compatta) e considero $\Norm{\cdot}_2\, : B_1\rightarrow \RR$. Questa funzione è continua rispetto alla metrica indotta da $\Norm{\cdot}_1$ infatti si conta:
$$
\Norm{u-v}_2\leq\sum_{i=1}^{d}\norm{u_i-v_i}\Norm{\vec{e_i}}_2\leq d\cdot\max_i\{\norm{u_i-v_i}\}\cdot \max_i\{\Norm{\vec{e_i}}_2\} 
$$
Quindi ammette $\max=M$ e $\min=m$. Per un generico $u\in B_1$: 
$$m\Norm{u}_1\leq \Norm{u}_2\leq M\Norm{u}_1$$
Inoltre se si prende $V$ come l' insieme delle successioni assolutamente convergenti le seguenti norme non sono equivalenti:
$$
\Norm{x_n}_1=\sup_k \norm{x_k} \mbox{      e       } \Norm{x_n}_2=\sum_{k\in\NN} 2^{-k}\norm{x_k}
$$
