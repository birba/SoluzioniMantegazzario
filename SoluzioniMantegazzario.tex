...
$$(\prod _{i=0} ^{n+1} a_i)^{1/n+1}=(\prod _{i=0} ^{n} a_i)^{1/n+1}\cdot a_{n+1}^{1/n+1}=(\prod _{i=0} ^{n} a_i^{1/n})^{n/n+1}\cdot a_{n+1}^{1/n+1}$$.
Si noti ora che gli esponenti dei due fattori sommano ad 1. Applichiamo dunque Young.
$$(\prod _{i=0} ^{n} a_i^{1/n})^{n/n+1}\cdot a_{n+1}^{1/n+1}\le \frac {n} {n+1} \prod _{i=0} ^{n} a_i^{1/n}+\frac {1} {n+1} a_{n+1}\le (\sum _{i=0} ^{n}a_i + a_{n+1})\cdot \frac{1}{n+1}=AM$$ dove nell'ultima disuguaglianza si è fatto uso dell'ipotesi induttiva di AM-GM su $n$ termini\\
HM-GM.
Anche qui si procede come prima cercando un modo furbo di applicare Young\\
QM-AM
Il passo base per $n=2$ è verificato. La tesi equivale a mostrare $$\sum_{i=0}^{n+1}a_i^2 \ge \frac {1}{n+1}((\sum_{i=0}^{n}a_i)+a_{n+1})^2= \frac {1}{n+1}((\sum_{i=0}^{n}a_i)^2+a_{n+1}^2+2a_{n+1}(\sum_{i=0}^{n}a_i))$$
Moltiplicando ambo i membri della disuguaglianza e spezzando alcuni addendi abbiamo $$n\sum_{i=0}^{n+1}a_i^2+\sum_{i=0}^{n}a_i^2+na_{n+1}^2\ge (\sum_{i=0}^{n}a_i^2)^2+2a_{n}\sum_{i=0}^{n}a_i^2$$
...
\ex{2.22}
Riscriviamo come $$(\sum_{i=0}^na_i)^2\le\sum_{i=0}^{n}a_i^2\Leftrightarrow2\sum_{i\neq j}^{n}a_ia_j\le (n-1)(\sum_{i=0}^{n}a_i^2)$$ 
...
Dato che la disugaglianza è omogenea prendiamo come ipotesi di comodo $$(\frac{1}{n}\sum_{i=0}^na_i)^p=1\Rightarrow \frac{1}{n}\sum_{i=0}^na_i=1$$.
Effettuiamo il seguente cambio di variabile: $a_i\rightarrow 1+b_i$, con $\sum_{i=0}^nb_i=0,b_i\ge 0$. Otteniamo $$\frac{1}{n}\sum_{i=0}^n(1+b_i)^p\ge 1$$ che è vera applicando ad ogni addendo di LHS la disuguaglianza di Bernoulli con le ipotesi sulla somma descritte in precedenza.
\ex{2.29} 
